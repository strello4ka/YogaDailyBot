...
> **Следующий поток (старт по дате, общий календарь 1–28):** полная спецификация и чеклист запуска — [`app/challenge/challenge-cohort-spec.md`](app/challenge/challenge-cohort-spec.md).
...
**Кто считается участником:** пользователи с `bot_mode = challenge`, заданным `challenge_start_id`, без паузы, блокировки и незавершённого онбординга. Отдельной таблицы «участники группы» нет.
...
**Типы сообщений** (по максимальному `challenge_day` среди активных участников):
...
| 29 (понедельник) | Финальные итоги: рейтинг с медалями 🥇🥈🥉 для топ-3, далее — 4, 5, … место |
...
Если `CHALLENGE_GROUP_CHAT_ID` пустой или нет активных участников челленджа — сводка молча пропускается.
...
- `/challenge_summary_preview` - отправляет сводку челленджа в групповой чат **сразу**, без ожидания 10:10. Не меняет флаги «уже отправлено сегодня» и «остановлено после финала» — удобно для проверки текста перед продом.
- `/challenge_summary_reset` - сбрасывает состояние сводок (`system_state`) после окончания челленджа, чтобы бот снова начал публиковать итоги для нового потока участников.
...
│   ├── challenge/     # Челлендж: команды, сводки в группу, админ-команды
