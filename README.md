...
|---|---|---|---|
| Выбор режима после `/start` (1-е) | Пользователь запустил `/start`, но режим не выбран (`bot_mode = pending`) | Через **1 час** после `/start` | `Чтобы бот начал работать, тебе нужно *выбрать режим*.\nОпределяйся: Daily или By mood — и погнали 🧡` |
| Выбор режима после `/start` (2-е) | То же условие (`bot_mode = pending`) | Через **24 часа** после `/start` | `Ты все еще не выбрал режим...\n\nНажми *Daily* или *By mood* — это займёт один миг` |
| Ввод времени в онбординге (1-е) | Пользователь выбрал Daily/Challenge, но не ввёл время | Через **1 час** после запроса времени | `Мы уже почти готовы начать долгий и счастливый путь к движению, осталось выбрать время, когда хочешь получать сообщения\n\n*Введи время в формате ЧЧ.ММ (например, 09.30)*` |
| Ввод времени в онбординге (2-е) | То же условие | Через **24 часа** после запроса времени | `Эй, мы так и не начали\nЕсли не выберешь время сегодня, придётся отложить старт..а жаль, тебя ждет отличное занятие!\n\n*Введи время в формате ЧЧ.ММ (например, 09.30)*` |
| Не нажата кнопка `✅ Я сделал!` (1-е) | Практика отправлена, но не отмечена как выполненная | Через **1 час** после отправки практики (только в тот же день и до 22:00 МСК) | **Случайный текст** из пула: `Кнопка «✅ Я сделал!» ждет нажатий` / `Прогресс сам себя не увеличит - как выполнишь практику, жми кнопку «✅ Я сделал!»` / `«✅ Я сделал!» — самая недооценённая кнопка в чате` |
| Не нажата кнопка `✅ Я сделал!` (2-е) | То же условие, и первое напоминание успевает до 19:30 | В **19:30 МСК** того же дня | **Случайный текст** из пула: `Практика все еще ждет тебя 🧡` / `Кажется ты кое-что забыл...Самое время расстилать коврик!` / `Похоже, практика осталась невыполненной, но все еще можно исправить` |
