...
- `/pause` - переключает режим паузы рассылки (ввести команду повторно, чтобы возобновить). В паузе ежедневные практики не отправляются, прогресс (день и позиция) сохраняется, а при возврате продолжается с того же места; если был активен челлендж, он завершается и после возобновления работает обычный режим.
