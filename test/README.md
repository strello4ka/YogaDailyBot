...
### Как понять, в каком окружении ты в терминале
...
- Вывод **`test/.env.test`** — в этой сессии терминала окружение тестовое (скрипты, которые читают `ENV_FILE`, пойдут в тестовую БД).
- Вывод **пустой** — переменная не задана, окружение по умолчанию (скрипты будут использовать `.env`, т.е. прод).

Чтобы включить тестовое окружение в текущем терминале:  
...
## 3.1. Загрузка практик в тестовую БД

Чтобы заполнить тестовую БД из тех же CSV, что и для прода (без риска задеть прод), используй скрипты из папки `test/`:
...
Запускать из корня проекта с активированным venv. В меню скрипта выбери «2. Обработать CSV файл», при запросе имени файла нажми Enter (подставятся файлы по умолчанию). Запись идёт только в тестовую БД.
