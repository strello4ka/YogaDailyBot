...
async def handle_progress_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text += social_line
    n = get_completed_count(user_id)
    streak = get_streak_days(user_id) if n else 0
    try:
        await send_rich_message(context.bot, update.effective_chat.id, _progress_blocks(user_id, n, streak, social_line))
    except Exception:
        logger.exception("Не удалось отправить Rich Message прогресса user=%s", user_id)
        await msg.reply_text(text, reply_markup=_progress_keyboard() if n else None, parse_mode='Markdown')
