...
def _simple_markdown_to_rich_text(value):
    """Convert the bold and italic markers used by the saved tips copy."""
    result = []
    for part in re.split(r"(\*.*?\*|_.*?_)", value, flags=re.DOTALL):
        if not part:
            continue
        if part.startswith("*") and part.endswith("*"):
            result.append({"type": "bold", "text": part[1:-1]})
        elif part.startswith("_") and part.endswith("_"):
            result.append({"type": "italic", "text": part[1:-1]})
        else:
            result.append(part)
    return result
...
def _tips_blocks():
    return [
        ...
        {"type": "details", "summary": "Почему плохо выполнять активные практики перед сном?", "blocks": [
            paragraph(_simple_markdown_to_rich_text(HELP_SLEEP_QUESTION_MARKDOWN))
        ]},
        paragraph("В любом случае делай так, как комфортно только тебе.\nПриятных занятий 🧡"),
        _back_button(),
    ]
...
HELP_SLEEP_QUESTION_MARKDOWN = (
    ...
    "*От этого:*\n"
    "🌀 *дольше засыпаешь*\n"
    "плюс появляются эндорфиновые мысли после тренировки из разряда "
    "\"завтра стану лучшей версией себя и выполню сто дел\", которые тоже не дают заснуть.\n"
    ...
    "Маркер для таких практик в боте: сложность — низкая."
)
