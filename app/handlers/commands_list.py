...
COMMANDS_HELP_TEXT = (
    "*📋 Все команды бота*\n\n"
    "*Для пользователей*\n"
    "`/start` — онбординг или подтверждение полного перезапуска\n"
    "`/help` — помощь, советы и рекомендации практик\n"
    "`/progress` — прогресс практик (в челлендже ещё N/28)\n"
    "`/favorite` — избранные практики\n"
    "`/practice` — подсказка по быстрым кнопкам практик\n"
    "`/schedule` — временная заглушка раздела расписания\n"
    "`/suggest` — предложить практику с YouTube\n"
    "`/donate` — подписка (пока существующая логика донатов)\n"
    "`/change_mode` — устаревший вход; ведёт к сценарию /start\n"
