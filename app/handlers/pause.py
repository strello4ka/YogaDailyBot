...
async def pause_toggle_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if notify_time is None:
        await message.reply_text(
            "Сначала настрой время для рассылки в режиме Daily (кнопка «Изменить время» или /start)."
        )
        return
    ...
    if is_paused_now:
        await message.reply_text(
            ...
            "Когда захочешь вернуться, просто жми кнопку *Пауза* на клавиатуре — продолжим с того же места",
            parse_mode='Markdown'
        )
