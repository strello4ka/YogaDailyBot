...
async def handle_tips_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    recommendations_text = (
        "Вот несколько простых рекомендаций, чтобы я стал твоим верным другом\n\n"
        "🌀 *1. Выдели время*\n"
        ...
        "Йога с ботом - это не гонка и не короткий марафон. Если не получилось позаниматься вчера, не нужно «нагонять» и выполнять несколько практик подряд. Никакой суеты и насилия над собой. Просто присоединяйся к сегодняшней практике, ведь она пришла именно сегодня не случайно.\n"
        "🌀 *4. YouTube и VPN*\n"
        ...
        "Мой проект только зарождается, поэтому так важен любой фитбек. Если хочешь сообщить об ошибке, предложить идею или просто общаться с другими пользователями бота, то заходи в [группу] (https://t.me/+ammNnozkY9kyNTJi) или пиши @strello4ka. Благодаря тебе YogaDailyBot станет лучше!\n\n"
        "🌀 *8. Комфорт превыше всего*\n" 
