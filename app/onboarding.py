...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    2. Пример практики с кнопкой "ХОЧУ"
    ...
    welcome_text_1 = (
        ...
        "Я твой YogaDailyBot, и я создан, чтобы мягко и ненавязчиво внедрить йогу в твою повседневную жизнь.\n\n"
        "Каждый день я буду присылать тебе короткую, но эффективную йога-практику без лишнего поиска и выбора:\n"
        "🌀 10-25 минут в день\n"
        "🌀 отборные практики разных уровней и направлений от моих любимых преподавателей с Youtube\n"
        "🌀 легко встроить в утро, обед или вечер\n"
        "🌀 практики подобраны так, что неделя включает в себя: 2-3 интенсивных занятия и другие интересные практики на расслабление, улучшение осанки и дыхания\n\n"
        "Выбор я беру на себя, тебе лишь остается расстелить коврик и нажать ▶️"
    )
    
    # Второе сообщение - пример практики с кнопкой
    welcome_text_2 = (
        "Вот как выглядит ежедневная практика:\n"
    ...
    await update.message.reply_text(welcome_text_1, parse_mode='Markdown')
    
    # Отправляем второе сообщение с кнопкой "ХОЧУ"
...
async def want_start_callback(update: Update, context: CallbackContext):
    ...
    time_input_text = (
        "Супер! Ты в деле\n"
        "Теперь давай выберем время, в которое ты хочешь получать сообщения, рассылка начнется с завтрашнего дня\n"
...
async def handle_time_input(update: Update, context: CallbackContext):
    ...
    success_text = (
        ...
        "А если не терпится начать прямо сейчас, жми и наслаждайся: [перейти к практике](https://youtu.be/oTzetTgYpSU?si=_V8LNx3i3Iq5zeoH)\n\n"
    ...
    menu_description_text = (
        ...
        "💡 **Предложить практику** - Делись своей любимой практикой: жми кнопку, и я с радостью добавлю ее в рассылку\n"
