...
def offer_text(value):
    return (
        f"🗓️ <b>Хочешь продолжить получать практики каждый день в {value}, как во время челленджа?</b>\n\n"
...
async def callback(update, context):
    ...
    if query.data == "pc:continue":
        await _save_schedule(update, state, state["time"])
        return
...
async def _save_schedule(update, state, value):
